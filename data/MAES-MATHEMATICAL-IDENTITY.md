...
Mae has 147 systems (92 core + 55 market), all wired, 4,536 tests passing. 157 holons, 425 triadic connections (217 core + 47 fractal + 55 bootstrap + 106 market, 0 bare dyads). Complete autopoietic loop with FEP-compliant prediction and TaskPool environment. Always-On Wave 1 adds MarketClock (time-aware source selection), daemon mode (wall-clock pacing), and signal persistence (convergence_alerter, somatic_anticipation, deception_detector survive restarts). Always-On Waves 2+3 add real-time streaming (Finnhub WebSocket), 24/7 crypto coverage (CoinGecko + CoinCap), pre-filtered insider trades (OpenInsider), institutional holdings (EDGAR 13F/13D), screener signals (FinViz unusual volume + short squeeze), and economic calendar suppression (FOMC/CPI/NFP confidence reduction). Progress:
