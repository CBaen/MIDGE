...
Data models (PoliticianProfile, CorrelationSignal) and member loading
are in politician_models.py (extracted for size compliance).
"""
...
# Data models live in politician_models (extracted for size compliance).
# Re-exported so existing callers see no import-path change.
from mae_core.market.edge.politician_models import (  # noqa: F401
    PoliticianProfile,
    CorrelationSignal,
    KNOWN_POLITICIANS,
    _load_congress_members,
)
...
class PoliticianTracker:
    ...
    def _calculate_confidence(self,
        ...
        if days_between < 0:  # Trade came before contract
            timing_bonus = min(0.3, abs(days_between) / 30 * 0.1)
